/**
 * VideoMorph Engine - Trim Operation Module
 */

import { TrimOperationOptions, EngineResult, ProgressCallback } from "../types/engine.types";
...
export async function executeTrimOperation(
  options: TrimOperationOptions,
  jobId: string,
  ...
  if (!inputFile) {
    throw new VideoMorphError("No input file provided for trimming.", "INVALID_INPUT");
  }
  ...
  const cleanOutputName = `trimmed_${Date.now()}.${options.outputFormat.toLowerCase()}`;
  ...
  const outputFilename =
    ...
    EngineDownloadService.generateOutputFilename(inputFile.name, options.outputFormat, "trimmed");
